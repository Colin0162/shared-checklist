...
```
src/
├─ App.jsx              ← '교통정리'(오케스트레이터): 데이터 로드·URL 라우팅·화면 조율만
├─ App.css              ← 거의 모든 디자인(색·여백·표 스타일 등)
├─ index.css            ← 색상 변수 모음(:root). 색 톤 바꾸려면 여기
├─ lib/
│   ├─ supabase.js      ← DB 접속 설정 (보통 안 건드림)
│   ├─ api.js           ← DB에 읽고/쓰는 함수 모음(로그인·게시글·항목·폴더·템플릿·계정·활동로그 등)
│   └─ constants.js     ← 상수: RATINGS(상중하), ddayLabel(D-day 계산)
├─ hooks/               ← 화면이 아니라 '로직 묶음'(재사용·App 슬림화)
│   ├─ useBoardItems.js ← 열린 게시글의 항목: 로드·실시간·체크/비고 저장·저장실패 재시도
...
└─ components/
    ...
    ├─ FolderView.jsx   ← 폴더/게시글 '목록 화면'(브레드크럼+폴더+게시글+새 폴더/게시글)
    ├─ FolderList.jsx   ← 폴더 목록 + '새 폴더'(공개/나만보기)
    ├─ BoardList.jsx    ← (폴더 안) 게시글 카드 목록 + 형식 라벨 + D-day + (관리자)삭제
    ├─ Checklist.jsx    ← 게시글 열었을 때: 진행률, 미완료/담당자 필터, 관리자, 인쇄, 📋기록, 항목들/표
    ├─ ActivityLog.jsx  ← '📋 기록' 모달: 누가·언제 체크/비고 했는지 최근 기록
    ├─ Item.jsx         ← 항목 한 줄(체크박스 / 상중하 / 할 일 텍스트 / 비고칸 / 담당자 / 저장실패 ↻)
...
## 6. 주소(URL) 구조 — 링크 공유·QR·뒤로가기
화면마다 주소가 따로 있어서 **링크 공유·북마크·QR**가 되고 **폰 뒤로가기**가 정상 동작해요.
- `/` : 홈(폴더 목록)
- `/folder/<폴더id>` : 그 폴더 안
- `/board/<게시글id>` : 그 게시글(입장 비번 있으면 먼저 비번 입력)

작동 방식(`App.jsx`): 주소를 따로 state로 들고 있지 않고 **주소에서 현재 위치를 계산(파생)**해요.
`react-router-dom`의 `useLocation`(지금 주소 읽기)·`useNavigate`(주소 바꾸기)를 씀.
폴더/게시글을 누르면 `navigate('/folder/..')`처럼 **주소만 바꾸면** 화면이 따라 바뀜.
- 새 게시글에 QR을 붙이고 싶으면: 그 게시글 주소(`/board/<id>`)를 QR 라이브러리로 그리면 끝.
